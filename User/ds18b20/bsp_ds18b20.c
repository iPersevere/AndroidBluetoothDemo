...
uint8_t DS18B20_Init(void)
{
	...
	//printf("DS18B20_GPIO_Config finished\n");
	
	DS18B20_DQ_1;
	
	//printf("DS18B20_DQ_1 finished\n");
	
	DS18B20_Rst();
	
	printf("DS18B20_Rst() finished\n");
...
static uint8_t DS18B20_Presence(void)
{
	...
	//SysTick_Init();
	printf("111pulse_time finished %d\n",pulse_time);
	/* 经过100us后，存在脉冲都还没有到来*/
	if( pulse_time >=100 )//100
		return 1;
	else
		pulse_time = 0;
	//SysTick_Init();
	/* 存在脉冲到来，且存在的时间不能超过240us */
	while( !DS18B20_DQ_IN() && pulse_time<240 )//240
	{
		pulse_time++;
		Delay_us(1);
	...
	//SysTick_Init();
	printf("222pulse_time finished %d\n",pulse_time);
	if( pulse_time >=240 )//240
		return 1;
	else
		return 0;
}//发现的问题：DS18B20_DQ_IN()一直为高电平，低电平一直未来，存在脉冲一直未到
...
void DS18B20_ReadId ( uint8_t * ds18b20_id )
{
	...
	printf("DS18B20_WriteByte finished");
	
	for ( uc = 0; uc < 8; uc ++ )
	  ds18b20_id [ uc ] = DS18B20_ReadByte();
	printf("DS18B20_ReadByte finished");
